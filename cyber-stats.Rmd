...
df <- read_excel(SQA_awards_filename, sheet="National_Progression_Awards", skip = 2) %>%
  filter(grepl(code_of_interest,Subject,fixed=TRUE)) %>%
  pivot_longer(cols = starts_with("Awarded Count "), names_prefix = "Awarded Count ", names_to= "year", values_to = "count") %>%
  mutate(count = as.numeric(count))

g <- ggplot(df, aes(x=year,y=count,fill=Level)) + theme(plot.title=element_text(size=14,face="bold"),axis.text=element_text(size=12), axis.title=element_text(size=12),strip.text.x = element_text(size = 14))
...
ggplotly(ggplot(main_longitudinal_df, aes(x=year,y=count)) + geom_col(fill = cbPalette[1]) + labs(y="Number of teachers") + my_theme + theme(legend.position = "none"))
...
# :TODO: it might eventually be worth dealing with the SFC College Statistics publication for FE students - some relevant data from 2018 onward
...
ggplotly(ggplot(table_10a_female , aes(x=year,y=Number)) + geom_col(fill=cbPalette[1]) + labs(y="Percentage of female students") + my_theme + scale_fill_manual(values=cbPalette) + facet_wrap(~Level,ncol=3))
...
ggplotly(ggplot(table_30, aes(x=year,y=as.integer(Number))) + geom_col(fill=cbPalette[1]) + labs(x = "academic year", y="Number qualifying") + my_theme)
...
### Computing students enrolled at Scottish universities, 2019/20 to 2021/22  
...
### Computing students enrolled at Scottish universities - by subject, 2021/22
```{r}
...
### Computing students enrolled at Scottish universities - by institution, 2021/22
...
table_54_HECoS_cyber <- table_54 %>% 
  ...
  group_by(CAH.level.subject,Academic.Year) %>%
...
table_26_Scottish_cs_by_mode <- table_26_Scottish_cs %>% 
  group_by(Mode.of.employment,Academic.Year) %>%
  summarise(Number=sum(Number))

ggplotly(ggplot(table_26_Scottish_cs_by_mode, aes(x=Academic.Year,y=Number)) + geom_col(aes(fill=Mode.of.employment)) + labs(x = "Academic year",fill="Mode of employment") + my_theme + scale_fill_manual(values=cbPalette))
...
Scottish_cs_total <- table_26_Scottish_cs %>% 
...
Scottish_cs_full_time <- Scottish_cs_total - Scottish_cs_part_time

UK_cs_total <- table_26 %>%
  filter(grepl("121", Category)) %>%
  filter(grepl("All", Country.of.HE.provider)) %>%
  filter(grepl("2021/22",Academic.Year)) %>%
  pull(Number) %>% 
  sum()
...
# data from an earlier section
Scottish_computing_FTE <- total_Scottish_computing_academics_df %>% 
  filter(grepl("2021/22",Academic.Year)) %>% 
  pull(Number) %>% 
  sum()
...
headline <- paste0(sprintf("%2.0f%%",(Scottish_cs_part_time/Scottish_cs_total)*100), " of Scottish computer science academics work part-time. This compares to ", sprintf("%2.0f%%",(Scottish_part_time/Scottish_total)*100), " for Scottish academics of all disciplines and ", sprintf("%2.0f%%",(UK_cs_part_time/UK_cs_total)*100), " for UK computing academics.\n  The average part-time academic contributes ", sprintf("%2.0f%%",((Scottish_computing_FTE - Scottish_cs_full_time) / Scottish_cs_part_time) *100), " of an FTE.\n")
...
table_26_Scottish_cs_by_nationality <- table_26_Scottish_cs %>% 
  group_by(Nationality,Academic.Year) %>%
  summarise(Number=sum(Number))

ggplotly(ggplot(table_26_Scottish_cs_by_nationality, aes(x=Academic.Year,y=Number)) + geom_col(aes(fill=Nationality)) + labs(x = "Academic year",fill="Nationality") + my_theme + scale_fill_manual(values=cbPalette))
...
ggplotly(ggplot(totals, aes(x=Academic.year,y=Value)) + geom_col(fill=cbPalette[1]) + labs(x = "Academic year",y= "Income (in thousands of pounds)") + my_theme)
